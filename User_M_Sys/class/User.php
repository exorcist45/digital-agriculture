...
class User extends dbConfig {	
    ...
    public function __construct(){
        if(!$this->dbConnect){ 		
            ...
            $conn = new mysqli('localhost', 'root', '', 'user management system');
		...
		if(!empty($_POST["register"]) && $_POST["email"] !='') {
			...
			if($isUserExist) {
				...
				if($userSaved) {
					...
					$link = "http://localhost/DigitalAgriculture/User_M_Sys/verify.php?authtoken=$authtoken";
					$toEmail = $_POST["email"];
					$subject = "Verify email to complete registration";
					$msg = "Hi there, click on this ".$link." to verify email to complete registration.";
					$msg = wordwrap($msg,70);
					$headers = "From: rockingashu912@gmail.com";
					if(mail($toEmail, $subject, $msg, $headers)) {
						$message = "Verification email sent to your email address. Please check email and verify to complete registration.";
					}
				} else {
	...
	public function resetPassword(){
		...
		if($_POST['email'] == '') {
			...
			if($numRows) {			
				...
				$link="http://localhost/DigitalAgriculture/User_M_Sys/reset_password.php?authtoken='.$authtoken.'";				
				$toEmail = $user['email'];
				$subject = "Reset your password on digital";
				$msg = "Hi there, click on this ".$link." to reset your password.";
				$msg = wordwrap($msg,70);
				$headers = "From: rockingashu912@gmail.com";
				if(mail($toEmail, $subject, $msg, $headers)) {
					$message =  nl2br("\n\n\nPassword reset link sent. \r\nPlease check your mailbox to reset password.");
				}				
			} else {
